...
class TripletLoss(BaseLoss):
    ...
    def forward(self, features=None, embeddings=None, labels=None, **kwargs):
        # 如果没有传入 features，则使用 embeddings
        if features is None:
            features = embeddings

        # 如果 features 仍然为 None 或者 labels 为 None，抛出错误
        if features is None or labels is None:
            raise ValueError("TripletLoss needs features/embeddings and labels")
